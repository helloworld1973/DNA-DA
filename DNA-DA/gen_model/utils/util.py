...
def log_confusion_matrix(conf_matrix, filename):
    """
    Logs the confusion matrix to a file.

    Args:
    - conf_matrix (numpy.ndarray): The confusion matrix to be logged.
    - filename (str): The name of the file to which the confusion matrix will be written.
    """
    # Convert the confusion matrix to a string
    conf_matrix_str = np.array2string(conf_matrix, separator=', ')
    log_and_print(f"Confusion Matrix:\n{conf_matrix_str}", filename)
